...
describe('StaticJsonDataProvider', () => {
  it('charge et valide les trois documents', async () => {
    ...
    const fetcher = vi.fn(async () => new Response(JSON.stringify(values.shift()), { status: 200 }));
    const provider = new StaticJsonDataProvider('/data', fetcher);
    expect((await provider.loadDataset()).observations).toHaveLength(1);
    expect((await provider.loadManifest()).observationCount).toBe(1);
    expect((await provider.loadQualityReport()).status).toBe('success');
  });

  it('remonte une erreur de chargement explicite', async () => {
    const provider = new StaticJsonDataProvider('/data', async () => new Response('', { status: 503 }));
