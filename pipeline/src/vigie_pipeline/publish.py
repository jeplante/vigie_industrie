...
def build_manifest(dataset: VigieDataset, generated_at: datetime | None = None) -> DatasetManifest:
    timestamp = generated_at or datetime.now(UTC)
    serialized = json.dumps(
        dataset.model_dump(mode="json", by_alias=True), ensure_ascii=False, sort_keys=True
    )
...
def publish_validated(
    ...
    manifest = build_manifest(dataset, quality_report.generated_at)
